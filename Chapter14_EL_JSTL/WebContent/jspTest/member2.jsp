...
<h1>c:set테그로 긴 속성이름을 짧은 변수로 대체하는 방법 </h1>
   <table style="0 auto;" border="1">
      ...
      <tr> <!-- 2번째 회원정보 출력(사용자가 입력한것) 47번줄 적어서 단축으로 가능하다 시발것아-->
     		<%-- <td>${membersMap.membersList[0].id }</td> --%>
			<td>${memberList2[0].id }</td>
			<td>${memberList2[0].pwd }</td>
			<td>${memberList2[0].name }</td>
			<td>${memberList2[0].email }</td>
      </tr>
      
    <tr> <!-- 3번째 회원정보 출력(손흥민)  이렇게 긴걸  47번줄 에서 적으면 단축 가능하다-->
         <%-- <td>${membersMap.membersList[1].id}</td> --%>
         <td>${memberList2[1].id}</td>
         <td>${memberList2[1].pwd}</td>
         <td>${memberList2[1].name}</td>
         <td>${memberList2[1].email}</td>
      </tr>
   </table>
</body>
</html>